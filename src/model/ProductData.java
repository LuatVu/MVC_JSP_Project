...
public class ProductData {
    ...
    // trả về danh sách sản phảm lấy ngẫu nhiên từ csdl (cho trang chủ)
    ...
    public static ArrayList<Products> getProductListByCategory(String categoryId) throws SQLException, ClassNotFoundException {
        ArrayList<Products> productList = new ArrayList<>();
        DbConnect connect = new DbConnect();
       String query = "SELECT PID, Img, PName, Products.CategoryId, CategoryName, PPrice, PDescription, SName, SAddress FROM Products, Category, Suppliers WHERE Products.SID = Suppliers.SID AND Category.CategoryId = Products.CategoryId AND Category.CategoryId = '" + categoryId + "'";
